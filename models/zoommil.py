...
class ZoomMIL(nn.Module):
    ...
    def forward(self, x):
        x1, x2, x3 = x
        x1, x2, x3 = align_patch_tensors(x1, x2, x3)               
...
def align_patch_tensors(x1, x2, x3, ratio=4):
    """
    Truncates patch tensors to exact multiples to satisfy ZoomMIL constraints.
    
    Args:
        x1, x2, x3 (torch.Tensor): Tensors of shape [batch, N, dim]
        ratio (int): The patch multiplier between consecutive magnifications. 
                     (e.g., 4 if moving from 5x to 10x with a constant patch size).
    Returns:
        tuple: Aligned tensors (x1, x2, x3)
    """
    # Get current patch counts (N dimension)
    N1 = x1.shape[1]
    N2 = x2.shape[1]
    N3 = x3.shape[1]
    
    # Find the bottleneck that dictates the maximum valid base patches at the lowest mag
    max_valid_N1_from_x1 = N1
    max_valid_N1_from_x2 = N2 // ratio
    max_valid_N1_from_x3 = N3 // (ratio * ratio)
    
    # The true base count is the minimum of these limits to ensure no out-of-bounds errors
    base_N1 = min(max_valid_N1_from_x1, max_valid_N1_from_x2, max_valid_N1_from_x3)
    
    # Calculate the exact mathematical multiples needed
    target_N1 = base_N1
    target_N2 = base_N1 * ratio
    target_N3 = base_N1 * (ratio * ratio)
    
    # Truncate tensors (dropping the minimum necessary patches from the end)
    x1_aligned = x1[:, :target_N1, :]
    x2_aligned = x2[:, :target_N2, :]
    x3_aligned = x3[:, :target_N3, :]
    
    # print(f"Original sizes: {N1}, {N2}, {N3}")
    # print(f"Aligned sizes:  {target_N1}, {target_N2}, {target_N3}")
    
    return x1_aligned, x2_aligned, x3_aligned
