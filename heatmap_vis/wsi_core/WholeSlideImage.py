...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        # print(vis_level)
        raw_coords = coords
        raw_psize = patch_size[0]
        print('!!!!!! r coords', raw_coords)
        print('!!!!!! r psize', patch_size)
        downsample = self.level_downsamples[vis_level]
        ...
        if binarize:
            ...
            threshold = 0.0
        print('!!!!!! th in vis heatmap', threshold)
        ...
        print(type(region_size))
        # overlay = np.full(np.flip(region_size), 0).astype(float)
        overlay = np.full(np.flip(region_size), 0).astype(np.float16)
        # print(overlay)
        ...
        del counter 
        if blur:
            print('USING blur (this function is replaced by torchvision)')
            h, w = overlay.shape
            # overlay = cv2.GaussianBlur(overlay,tuple((patch_size * (1-overlap)).astype(int) * 2 +1),0)  
            gaussian = transforms.GaussianBlur(tuple((patch_size * (1-overlap)).astype(int) * 2 +1),0.2)
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            overlay = torch.from_numpy(overlay).view(1,1,w,h).to(device)
            overlay = gaussian(overlay).view(h,w).detach().cpu().numpy()
        ...
        print('blank canvas:', blank_canvas)
        if not blank_canvas:
        ...
        boxes_for_qupath_vis = []
        for idx in range(len(coords)):
            if (idx + 1) % twenty_percent_chunk == 0:
                print('progress: {}/{}'.format(idx, len(coords)))
            score = scores[idx]
            coord = coords[idx]
            raw_coord = raw_coords[idx]
            # the threshold here is for 'binarizing' not heatmap
            box_counter = 1
            if score >= threshold:
                boxes_for_qupath_vis.append((raw_coord[0], raw_coord[1], raw_psize, raw_psize, str(box_counter)))
                box_counter += 1
                # attention block
        ...
        return img, boxes_for_qupath_vis
