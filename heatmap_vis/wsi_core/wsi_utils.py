...
def to_percentiles(scores):
    print('before process scores:', scores, len(scores))
    from scipy.stats import rankdata
    percentile_scores = rankdata(scores, 'average')/len(scores) * 100
    print('processed scores:', scores)
    # calibrated_overall_score = np.mean(scores * (scores > opt_th))
