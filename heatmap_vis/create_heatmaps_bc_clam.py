...
from vis_utils.heatmap_utils import initialize_wsi, drawHeatmap, compute_from_patches, compute_from_patches_fast
...
def infer_single_slide(model, features, label, reverse_label_dict, opt_th, k=1):
    features = features.to(device)
    print(model)
    print(model.__class__)
    with torch.no_grad():
        A, logits = model(features, vis_heatmap=True)
        # print('logits', logits)
    ...
    print('Original attn map size:', A, A.shape)
...
if __name__ == '__main__':
    ...
    for i in range(len(process_stack)):
        ...
        Y_hats, Y_hats_str, Y_probs, A, Y_hat_calibrated = infer_single_slide(model, features, label, reverse_label_dict, opt_th,
                                                            exp_args.n_classes)
        ...
        if heatmap_args.calc_heatmap:
            ...
            compute_from_patches_fast(wsi_object=wsi_object, model=model, features=features, coords=coords, clam_pred=None, attn_save_path=save_path)
