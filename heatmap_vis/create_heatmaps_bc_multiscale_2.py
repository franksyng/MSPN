...
# from preprocessing.backbones.gigapath import gigapath_backbone
...
def infer_single_slide(model, features, label, reverse_label_dict, opt_th, k=1):
    ...
    with torch.no_grad():
        A, logits = model(features, vis_coarse_map=True)
        A = A[0]
        # print('logits', logits)
...
if __name__ == '__main__':
    ...
    # emb = .encode_image(image, proj_contrast=False, normalize=False)
    print('Done!')
    ...
    for i in range(len(process_stack)):
        ...
        if heatmap_args.calc_heatmap:
            ...
            compute_from_patches(wsi_object=wsi_object, clam_pred=Y_hats[0], model=model,
                                 feature_extractor=feature_extractors, batch_size=exp_args.batch_size, **wsi_kwargs,
